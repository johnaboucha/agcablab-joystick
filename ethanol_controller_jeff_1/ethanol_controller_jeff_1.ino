...
void loop() {
  ...
  if (throttle_val > 500) { throttle_val = 500; }
  if (throttle_val < 270) { throttle_val = 270; }
  ...
  joySt.xAxis = (map(steering_val, 160, 510, 0, 255));
  //Serial.println(throttle_val);
  if (brake_val > 270) {
    joySt.xRotAxis = (map(brake_val, 260, 480, 126, 255));
    //Serial.println("brake");
  } else{// (throttle_val > 280) {
    joySt.xRotAxis = (map(throttle_val, 270, 500, 128, 0));
    //Serial.println("gas");
  } //else {
   //joySt.xRotAxis = 0; 
